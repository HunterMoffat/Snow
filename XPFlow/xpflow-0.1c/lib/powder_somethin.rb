...
module XPFlow;
    class Powder
        ...
        def startExperiment(*args)
            if args.size == 3
               # $LOG.log("Starting Experiment: #{$EXPERIMENT}, using profile #{$PROFILE} in project #{$PROJECT}")
                #$LOG.info("Starting Experiment: #{$EXPERIMENT}, using profile #{$PROFILE} in project #{$PROJECT}")
    ...
    class Library < ActivityLibrary
        ...
        def startExperiment(*args)
            ...
            if args.size == 3
                ...
                exp = args[1]
                prof = ags[2]
                log "Starting the experiment with 3 args!\n"
